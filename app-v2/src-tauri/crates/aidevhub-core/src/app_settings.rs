...
pub fn save_settings(paths: &AppPaths, settings: AppSettings) -> Result<AppSettings, AppError> {
    fs::create_dir_all(&paths.app_local_data_dir).map_err(|e| {
        AppError::new(
            "IO_ERROR",
            format!("mkdir {}: {e}", paths.app_local_data_dir.display()),
        )
    })?;
    ...
    fs::write(&path, text)
        .map_err(|e| AppError::new("IO_ERROR", format!("write {}: {e}", path.display())))?;
    Ok(settings)
}
